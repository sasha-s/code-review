...
Task: write the final review from the verified ledger, transition challenge,
branch/consequence audit, and candidate selector. Do not include benchmark or
golden language. Treat the `09-candidate-selector`
...
- Every candidate id should be included, rejected with source fact, or named as
  unresolved.
- Every `09-candidate-selector` candidate with `disposition: include_finding`
  must appear in Findings.
- Every `09-candidate-selector` candidate with `disposition: include_boundary`
