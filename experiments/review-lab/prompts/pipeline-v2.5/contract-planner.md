...
- For changed resource bundles, locale/message files, copied UI strings, or
  generated metadata, treat the hunk as runtime when the value is displayed,
  parsed, matched, loaded by key, or used by tests. Create cards for expected
  language/script, neighboring/base-locale consistency, message key spelling,
  placeholder/interpolation grammar, translated HTML/anchor grammar, and loader
  lookup shape when those dimensions appear in the changed code.
...
- For new or renamed functions, methods, command names, resource keys, generated
  metadata keys, or public/test-visible identifiers, create a naming/API contract
  when the name must match callers, reflection, diagnostics, docs, tests, or a
  domain term in adjacent source. Seed a candidate if the changed spelling looks
  inconsistent with the target term or call-site spelling.
...
1. Contract Cards
   ...
   - For each changed runtime operation, include:
     ...
     - proof dimensions that must be closed: reachability, old-vs-new,
       ...
       operation itself
     - suggested reviewer focus: browser/render, resource/locale, remote/import,
       state/API, or multiple
...
3. Coverage Map
   ...
   - Non-runtime hunks marked as tests, style, migration-only, dependency-only,
     or docs. Do not mark locale/resource hunks non-runtime merely because they
     are data files; first decide whether the values are displayed, parsed,
     loaded, matched, or used as API/key names.
