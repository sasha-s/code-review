...
- For naming/API contracts, verify changed identifier spelling against call
  sites, reflection/loader paths, generated metadata, diagnostics, tests, and
  adjacent domain terms. Do not report cosmetic spelling only; keep it when the
  name is API-visible, test-visible, diagnostic-visible, or likely to hide a
  wrong call/lookup contract.
