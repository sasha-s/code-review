...
- Verified-candidate carry audit: every candidate marked verified finding or
  ...
  finding exists.
- Consequence preservation audit: final prose must state each carried
  candidate's concrete consequence. Mentioning a related code smell is not enough
  if the candidate consequence is an auth denial, bad persisted token, runtime
  parser/schema failure, missing-record behavior, timestamp-window mismatch,
  user-visible wrong locale/script, or security boundary weakening.

Output sections:

0. Verified Candidate Carry Matrix
   - Copy every `Must Appear In Final? = yes` row from the transition
     challenger.
   - Add a `Final Section` column naming the finding or boundary-concern title
     where the candidate appears.
   - If a carried candidate is not in final prose, mark `Final Section` as
     `REJECTED HERE` and give the exact rejecting source fact.
...
Do not drop medium/high verified candidates to keep the review short. If there
are many findings, keep concise one-paragraph findings rather than omitting
distinct issue classes.
