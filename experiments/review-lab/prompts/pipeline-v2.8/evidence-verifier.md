...
- For sentinel errors and error taxonomy, verify each named error independently:
  source branch that creates it, caller branch that treats it specially, whether
  it is logged/suppressed/returned, and the user or state consequence. Do not let
  a nearby transient-error finding hide a sentinel-error denial or retry change.
- For time-window/current-time candidates, list the lower bound, upper bound,
  persisted timestamp, generated timestamp, timezone basis, and caller source for
  every compared value. A direct-caller timestamp concern and a UTC-vs-current
  time concern are separate unless source proves the same branch and same
  consequence.
- For repeated-element matchers, parsers, sanitizers, placeholder verifiers, and
  resource validators, inspect iterator/cursor advancement and group/capture use
  for zero, one, extra, missing, duplicate, and reordered elements. Tests for a
  single changed element are evidence, not proof, unless they cover the same
  cardinality and failed-advance branch.
