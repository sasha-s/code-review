...
3. Required Follow-Up Checks
   ...
   - Request follow-up when a repeated-element parser/verifier has not proven
     extra, missing, duplicate, reordered, or failed-advance cases.
   - Request follow-up when a sentinel error is mentioned but not traced to the
     caller branch that logs, suppresses, retries, denies, or returns it.
   - Request follow-up when time-window reasoning does not name the exact time
     basis for each compared value.
