...
- Sentinel-error audit: if a candidate mentions a named error/sentinel/status,
  final text must say where that value is created and exactly how the caller
  handles it. Keep a denial/returned-error branch separate from log-only,
  suppressed, retry, cache, and transient-error branches.
- Time-basis audit: for every time-window finding or concern, state the exact
  lower-bound source, upper-bound source, persisted timestamp source, timezone
  basis, and whether the timestamp is caller-supplied or server-generated. Do not
  substitute generic stale-time language for a specific basis mismatch.
- Repeated-element audit: for sanitizer/parser/matcher/placeholder/resource
  checks, final text must state whether zero, one, extra, missing, duplicate, and
  reordered elements were source-proven or left as gaps. If source uses a cursor,
  iterator, matcher group, or capture after a failed advance, include that exact
  branch as a finding or boundary concern.
